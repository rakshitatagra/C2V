...
class _WaitingScreenState extends State<WaitingScreen> {
  ...
  // Tracks which TTS announcements already played
  // so they don't repeat on every GPS update
  ...
  void initState() {
    ...
    _startBLEScan();
    tts.speak('Waiting for bus ${widget.route}. '
  ...
  void _listenToLocation() async {
    ...
    // Listen for bus location updates from Firebase (sent by simulator app)
    _locationSub = db.child('location').onValue.listen((event) async {
      ...
      try {
        ...
        // GPS unavailable (indoor/demo room) — use fixed reference point
        userPos = Position(
          latitude: 28.6139,
          longitude: 77.2090,
      ...
      if (!mounted) return;
      setState(() => distanceText = '${dist.toInt()} meters');

      // 1500m — alert driver first time
      if (dist < 1500 && !_announced.contains('1500')) {
        ...
        await tts.speak('Bus ${widget.route} is entering your area. '
      ...
      if (dist < 500 && !_announced.contains('500')) {
        _announced.add('500');
        await tts
            .speak('Bus is 500 meters away. Please get ready to board.');
        Vibration.vibrate(duration: 300);
      }
      ...
      if (dist < 150 && !_announced.contains('150')) {
        _announced.add('150');
        await tts.speak('Bus is very close, about 150 meters. '
            'Please move to the boarding area.');
        Vibration.vibrate(pattern: [0, 400, 200, 400]);
      }

      // 80m — bus arriving, trigger ESP32 fast blink + BLE broadcast
      if (dist < 80 && !_announced.contains('arrived')) {
        ...
        await tts.speak('Bus is arriving now! Stay at the stop.');
  ...
  void _startBLEScan() async {
    // BUG FIX: Original code used Serial.println() which is Arduino syntax
    // Corrected to debugPrint() which is proper Dart/Flutter
    ...
    FlutterBluePlus.scanResults.listen((results) async {
      for (ScanResult r in results) {
        ...
        if (deviceName == targetName && !_announced.contains('ble')) {
          _announced.add('ble');
          ...
          await tts.speak('Bus ${widget.route} confirmed via Bluetooth! '
          ...
          if (mounted) {
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                builder: (_) => ArrivedScreen(route: widget.route),
              ),
            );
          }
        }
  ...
  void _listenToAcknowledgement() {
    _ackSub =
      ...
      if (!mounted || event.snapshot.value == null) return;
      if (event.snapshot.value == true && !driverAcknowledged) {
        setState(() {
          driverAcknowledged = true;
          statusText = 'Driver confirmed your stop!';
          statusColor = Colors.green;
        });
        await tts.speak('The driver has confirmed your stop request. '
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Icon(Icons.directions_bus, size: 90, color: statusColor),
              ...
              Semantics(
                liveRegion: true, // TalkBack announces changes automatically
                child: Text(
                  statusText,
                  style: TextStyle(
                      fontSize: 26,
...
class _StatusRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Icon(icon,
            color: active ? Colors.green : Colors.grey[700], size: 22),
